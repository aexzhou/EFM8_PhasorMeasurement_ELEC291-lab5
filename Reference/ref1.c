...
void main (void)
{
	...
	while(1)
	{
	    //*FOR HALF PERIOD MEASUREMENT
	    
	    // Start tracking the reference signal
		ADC0MX=QFP32_MUX_P0_1;
		ADINT = 0;
		ADBUSY=1;
		while (!ADINT); // Wait for conversion to complete
		// Reset the timer
		...
		//intf ("V@P2.2=%7.5fV, V@P2.3=%7.5fV, V@P2.4=%7.5fV, V@P2.5=%7.5fV\r", v[0], v[1], v[2], v[3]);
		waitms(500);
	
		//addition
		sprintf(buffer,"%f/r",half_period);
		LCDprint(buffer,1,1);
	 }  
}	
